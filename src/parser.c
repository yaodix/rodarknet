...
{
    int coords = option_find_int(options, "coords", 5);
    int classes = option_find_int(options, "classes", 20);
    int num = option_find_int(options, "num", 6);     //anchor 的个数
    int max_boxes = option_find_int_quiet(options, "max", 90);
    ...
    char *a = option_find_str(options, "anchors", 0);   //读取anchors 配置
    if(a){
        int len = strlen(a);
        int n = 1;
        int i;
        ...
        for(i = 0; i < n && i < num*3; ++i){
            float bias = atof(a);
            l.biases[i] = bias;
            a = strchr(a, ',')+1;   //char *strchr(const char *str, int c) 在参数 str 所指向的字符串中搜索第一次出现字符 c(一个无符号字符)的位置。
        }
    }
    // l.biases 存放了anchor的数值
    return l;
}
...
void parse_net_options(list *options, network *net)
{
    ...
    net->aspect = option_find_float_quiet(options, "aspect", 1);   //aspects  :设置方位，扩充数据
...
{
    list *sections = read_cfg(filename);   //一个section存储了CNN所需的参数
    node *n = sections->front;       //取出每个节点
    if(!n) error("Config file has no sections");
    network net = make_network(sections->size - 1);   //分配网络存储空间大小
    net.gpu_index = gpu_index;
    size_params params;

    section *s = (section *)n->val;
    list *options = s->options;    //  kvp,每层的一些具体配置
    if(!is_network(s)) error("First section must be [net] or [network]");
    parse_net_options(options, &net);       //解析[net]中的内容

    params.h = net.h;   //cfg 文件中的height  , width , channel  ,分别为416,416,3等
    params.w = net.w;
    params.c = net.c;
    ...
    while(n){  //循环解析 网络配置
        ...
        if(lt == CONVOLUTIONAL){
            ...
            l = parse_region(options, params);   //v2的输出层, 重要
        }else if (lt == YOLO) {
...
list *read_cfg(char *filename)
{
    ...
    while((line=fgetl(file)) != 0){
        ++ nu;
        strip(line);                                 //去除line中的空格
        switch(line[0]){          //判断第一个字符
            case '[':
