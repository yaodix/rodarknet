...
{
    rbox_label *rboxes = calloc(1, sizeof(rbox_label));
//*
    FILE *file = fopen(filename, "r");
    if (!file) {
        printf("Can't open label file. (This can be normal only if you use MSCOCO) \n");
    ...
    int id;
    int count = 0;
    while (fscanf(file, "%d %f %f %f %f %f", &id, &x, &y, &w, &h , &a) == 6) {     //读出参数
       rboxes = realloc(rboxes, (count + 1) * sizeof(rbox_label));
       rboxes[count].id = id;
       rboxes[count].x = x;
       rboxes[count].y = y;
       rboxes[count].h = h;
       rboxes[count].w = w;
       if (a - 0.001 <0.0001) a = 0.001;
       if (a - 3.1415 >0.00001) a = 3.1415;
       rboxes[count].a = a;
      ...
      // printf(filename); printf("\n");
       //printf("(x,y)=(%f,%f) w x h = %f,%f a = %f \n", rb.x,rb.y,rb.w,rb.h,rb.a);
       pointf ver[4];
       getRboxVertices(rb, ver);
       
       rboxes[count].ptlx = ver[0].x;
       rboxes[count].ptly = ver[0].y;
       rboxes[count].ptrx = ver[1].x;
       rboxes[count].ptry = ver[1].y;
       rboxes[count].pdrx = ver[2].x;
       rboxes[count].pdry = ver[2].y;
       rboxes[count].pdlx = ver[3].x;
       rboxes[count].pdly = ver[3].y;
...
void randomize_rboxes(rbox_label *b, int n)
{
    int i;
    for (i = 0; i < n; ++i) {
        rbox_label swap = b[i];
...
void correct_rboxes(rbox_label *boxes, int n, float dx, float dy, float sx, float sy, int flip)
{
    int i;
    for (i = 0; i < n; ++i) {
        if (boxes[i].x == 0 && boxes[i].y == 0) {
        ...
        boxes[i].ptlx = boxes[i].ptlx * sx - dx;
        boxes[i].ptly = boxes[i].ptly * sy - dy;

        boxes[i].ptrx = boxes[i].ptrx * sx - dx;
        boxes[i].ptry = boxes[i].ptry * sy - dy;

        boxes[i].pdrx = boxes[i].pdrx * sx - dx;
        boxes[i].pdry = boxes[i].pdry * sy - dy;

        boxes[i].pdlx = boxes[i].pdlx * sx - dx;
        boxes[i].pdly = boxes[i].pdly * sy - dy;
    ...
    //        boxes[i].right = 1. - swap;
     //   }
        ...
        boxes[i].ptlx = constrain(0, 1, boxes[i].ptlx);
        boxes[i].ptly = constrain(0, 1, boxes[i].ptly);

        boxes[i].ptrx = constrain(0, 1, boxes[i].ptrx);
        boxes[i].ptry = constrain(0, 1, boxes[i].ptry);

        boxes[i].pdrx = constrain(0, 1, boxes[i].pdrx);
        boxes[i].pdry = constrain(0, 1, boxes[i].pdry);

        boxes[i].pdlx = constrain(0, 1, boxes[i].pdlx);
        boxes[i].pdly = constrain(0, 1, boxes[i].pdly);

        boxes[i].x = (boxes[i].ptlx + boxes[i].pdrx) / 2;
        boxes[i].y = (boxes[i].ptly + boxes[i].pdry) / 2;
        boxes[i].w = sqrtf(powf((boxes[i].ptlx - boxes[i].ptrx),2)+powf((boxes[i].ptly - boxes[i].ptry), 2));
        boxes[i].h = sqrtf(powf((boxes[i].ptrx - boxes[i].pdrx), 2)+ powf((boxes[i].ptry - boxes[i].pdry), 2));
...
{
    char labelpath[4096];
    replace_image_to_label(path, labelpath);    //图片文件名转换位对应标签文件名
   // printf(labelpath);
    int count = 0;
    int i;
    ...
    if (small_object == 1) {
        for (i = 0; i < count; ++i) {
            if (rboxes[i].w < lowest_w) rboxes[i].w = lowest_w;
            if (rboxes[i].h < lowest_h) rboxes[i].h = lowest_h;
        }
    }
    //**************************************/
    randomize_rboxes(rboxes, count);
    correct_rboxes(rboxes, count, dx, dy, sx, sy, flip);   
    ...
    for (i = 0; i < count; ++i) {
        ...
        truth[(i-sub)*6+0] = x;   //存到truth中，格式为x、y、w、h、a、id
        truth[(i-sub)*6+1] = y;
        truth[(i-sub)*6+2] = w;
        truth[(i-sub)*6+3] = h;
        truth[(i-sub)*6+4] = a;
...
{
    c = c ? c : 3;
    char **random_paths = get_random_paths(paths, n, m);   //  提取n (一次训练的图片数量)
    ...
    d.y = make_matrix(n, 6*boxes);  //  rot -- 6
    for(i = 0; i < n; ++i){
        const char *filename = random_paths[i];
        //printf(filename);                                               //打印图片名
        int flag = (c >= 3);
        IplImage *src;
...
void *load_thread(void *ptr)
{
    ...
    } else if (a.type == DETECTION_DATA){  //YoloV2 用
...
void *load_threads(void *ptr)
{
    ...
    for(i = 0; i < args.threads; ++i){
        ...
        threads[i] = load_data_in_thread(args);   //--->load_data_in_thread -->load_thread --> a.type == DETECTION_DATA ,
    }
    for(i = 0; i < args.threads; ++i){
...
pthread_t load_data(load_args args)
{
    ...
    struct load_args *ptr = calloc(1, sizeof(struct load_args));
    *ptr = args;
    if(pthread_create(&thread, 0, load_threads, ptr)) error("Thread creation failed");     //数据加载线程的线程
    return thread;
}
...
void get_next_batch(data d, int n, int offset, float *X, float *y)
{
    int j;
    for(j = 0; j < n; ++j){
        //offset 就是第几个batch(i*batch)了，j表示的是每个batch的第几个样本(图像)
        int index = offset + j;
        memcpy(X+j*d.X.cols, d.X.vals[index], d.X.cols*sizeof(float));
        memcpy(y+j*d.y.cols, d.y.vals[index], d.y.cols*sizeof(float));
    }
