...
{
    list *options = read_data_cfg(datacfg);      //解析obj.data 文件  内容是 kvp
    char *train_images = option_find_str(options, "train", "data/train.list");  //解析出训练图片文件名  如 data/obj.data中 train  = data/train.txt
    char *backup_directory = option_find_str(options, "backup", "/backup/");
    
    srand(time(0));
    char *base = basecfg(cfgfile);   //  提取cfg文件的名字，不包括.cfg
    printf("%s\n", base);
    float avg_loss = -1;
    network *nets = calloc(ngpus, sizeof(network));  //根据gpu数分配内存
...
#endif
        nets[i] = parse_network_cfg(cfgfile);  // 解析网络结构
        if(weightfile){
    ...
    layer l = net.layers[net.n - 1];   //最后的region层

    int classes = l.classes;
    float jitter = l.jitter;

    list *plist = get_paths(train_images);   //解析出所有图片名
    //int N = plist->size;
    ...
    load_args args = {0};  //加载训练图片用到的一些参数
    args.w = net.w;
    args.h = net.h;
    args.c = net.c;
    args.paths = paths;      //训练图片路径
    args.n = imgs;             //一次载入到显存的图片数量
    args.m = plist->size;   //  图片集总数量
    args.classes = classes;
    args.flip = net.flip;
    args.jitter = jitter;
    args.num_boxes = l.max_boxes;   //最多的groud truth数
    ...
    pthread_t load_thread = load_data(args);     //加载数据
    double time;
    int count = 0;
    //while(i*imgs < N*120){
...
#ifdef OPENCV
            cvSaveImage("loss.jpg", img, 0);   //每100 batch 保存训练loss图像
#endif
        }
        free_data(train);
    }  // while(get_current_batch(net) < net.max_batches){  .. }
...
int detectionsR_comparator(const void *pa, const void *pb)
{
    rbox_prob a = *(rbox_prob *)pa;
    rbox_prob b = *(rbox_prob *)pb;
    float diff = a.p - b.p;
    if (diff < 0) return 1;
    else if (diff > 0) return -1;
    return 0;
...
void validate_detector_map(char *datacfg, char *cfgfile, char *weightfile, float thresh_calc_avg_iou)
{
    ...
    const float angle_thresh = CV_PI / 12.;

    int nthreads = 1;        //in src its 4,
    image *val = calloc(nthreads, sizeof(image));
    image *val_resized = calloc(nthreads, sizeof(image));
    image *buf = calloc(nthreads, sizeof(image));
    image *buf_resized = calloc(nthreads, sizeof(image));
   ...
   // const float thresh_calc_avg_iou = 0.7;
    float avg_iou = 0;
    int tp_for_thresh = 0;
    int fp_for_thresh = 0;

    rbox_prob *detections = calloc(1, sizeof(rbox_prob));
    int detections_count = 0;
    int unique_truth_count = 0;

    float angle_diffs[30];
    char imgPaths[4000][100] = { ""};
    float all_angle_diffs[4000] = { -1. };
    float all_angle_diffs_base[4000] = { -1. };
    int  angle_cnt = 0;
    float angle_diff_thresh = (10. / 180)*CV_PI;;
    int  angle_big_cnt = 0;
    float all_angle_diff = 0.;
    float angle_diff = 0.;
    ...
    for (i = nthreads; i < m + nthreads; i += nthreads) {
        ...
        for (t = 0; t < nthreads && i + t - nthreads < m; ++t) {
            ...
            int letterbox = (args.type == LETTERBOX_DATA);
            float hier_thresh = 0;
            detectionR *dets = get_network_rboxes(&net, 1, 1, thresh, hier_thresh, 0, 0, &nboxes, letterbox);
            //detection *dets = get_network_boxes(&net, val[t].w, val[t].h, thresh, hier_thresh, 0, 1, &nboxes, letterbox); // for letterbox=1
            if (nms) do_rnms_sort(dets, nboxes, l.classes, nms);  //计算出图像中的预测框,经过最大值抑制

            char labelpath[4096];
            replace_image_to_label(path, labelpath);
            int num_labels = 0;
            rbox_label *truth = read_rboxes(labelpath, &num_labels);   //num_labels是几个真实标注框
            ...
            if (paths_dif)
            {
                ...
                truth_dif = read_rboxes(labelpath_dif, &num_labels_dif);
            }

            const int checkpoint_detections_count = detections_count;

            for (i = 0; i < nboxes; ++i) {     //遍历预测结果box
                int class_id;
                for (class_id = 0; class_id < classes; ++class_id) {  //按种类遍历
                    float prob = dets[i].prob[class_id];
                    if (prob > 0) {
                        detections_count++;
                        detections = realloc(detections, detections_count * sizeof(rbox_prob));
                        ...
                        int truth_index = -1;
                        int angle_truth_index = -1; //为计算角度偏差而设的索引遍量
                        rbox truth_rbox[4000]  ;
                        float max_iou = 0;
                        for (j = 0; j < num_labels; ++j)   //num_labels: 标注的ground truth 个数
                        {
                            rbox t = { truth[j].x, truth[j].y, truth[j].w, truth[j].h, truth[j].a };
                            float current_iou = rbox_iou(dets[i].bbox, t);
                         //  printf(" IoU = %f, prob = %f, class_id = %d, truth[j].id = %d \n", rbox_iou(dets[i].bbox, t), prob, class_id, truth[j].id);

                            if (current_iou > iou_thresh && class_id == truth[j].id && fabsf(dets[i].bbox.a - t.a ) < angle_thresh*1.5) {   // 暂未加角度限制
                                if (current_iou > max_iou) {
                                    max_iou = current_iou;
                                    truth_index = unique_truth_count + j;       //找到预测框在标注框中对应的索引，索引是累加已读取的标注框
                                    truth_rbox[truth_index] = t;
                                }
                            }
                        ...
                        if (truth_index > -1) {
                            ...
                            //printf("1   pred :(x,y)=(%f,%f) w x h = %f,%f a = %f \n",
                            //    detections[detections_count - 1].b.x, detections[detections_count - 1].b.y, detections[detections_count - 1].b.w, detections[detections_count - 1].b.h, detections[detections_count - 1].b.a);
                            //printf("12 truth :(x,y)=(%f,%f) w x h = %f,%f a = %f \n",
                            //    truth_rbox[truth_index].x, truth_rbox[truth_index].y, truth_rbox[truth_index].w, truth_rbox[truth_index].h, truth_rbox[truth_index].a);
                            angle_diff = detections[detections_count-1].b.a - truth_rbox[truth_index].a;
                            if (fabsf(angle_diff) > angle_diff_thresh)
                            {
                               // strcpy(imgPaths[i], labelpath);   // 对指针 错误内容均为最后一个labelPath
                                printf("imgPaths = %s ", labelpath);
                                all_angle_diff += fabsf(angle_diff);
                                all_angle_diffs[truth_index] = angle_diff;
                                all_angle_diffs_base[truth_index] = truth_rbox[truth_index].a;
                                printf("base angle = %.2f,angle diff = %.2f \n", truth_rbox[truth_index].a* 180. / CV_PI, angle_diff* 180. / CV_PI);
                            ...
                            for (j = 0; j < num_labels_dif; ++j) {
                                rbox t = { truth_dif[j].x, truth_dif[j].y, truth_dif[j].w, truth_dif[j].h,truth_dif[j].a };
                                float current_iou = rbox_iou(dets[i].bbox, t);
                        ...
                        if (prob > thresh_calc_avg_iou) {
                            ...
                            for (z = checkpoint_detections_count; z < detections_count-1; ++z)
                                if (detections[z].unique_truth_index == truth_index) {
                                    found = 1;     
                                    printf("found = 1\n");
                                    //printf("pred :(x,y)=(%f,%f) w x h = %f,%f a = %f \n", 
                                    //    detections[z].b.x, detections[z].b.y, detections[z].b.w, detections[z].b.h, detections[z].b.a);
                                    //printf(" truth :(x,y)=(%f,%f) w x h = %f,%f a = %f \n",
                                    //    truth_rbox[truth_index].x, truth_rbox[truth_index].y, truth_rbox[truth_index].w, truth_rbox[truth_index].h, truth_rbox[truth_index].a);
                                    
                                    break;
                                }

                            if(truth_index > -1 && found == 0) {  //? found == 1
                                avg_iou += max_iou;
                                ++tp_for_thresh;
                            }
                            else
    ...
    //for (int i=0;i<unique_truth_count;i++)
    //{
    //    if(fabsf(all_angle_diffs[i]) > angle_diff_thresh )
    //    {
    //        printf(imgPaths[i]); printf(" ");// printf("truth_index = %d ",i);
    //        printf("base angle = %.2f,angle diff = %.2f \n", all_angle_diffs_base[i] * 180. / CV_PI, all_angle_diffs[i] * 180. / CV_PI); 
    //        angle_big_cnt++;
    //    }
    //}
    printf("over angle thresh %.3f  rbox cnt = %d ,ratio is %.2f, average angle diff = %.2f\n", 
                 angle_diff_thresh*180/CV_PI,   angle_cnt,    (float)angle_cnt/unique_truth_count,  (all_angle_diff/angle_cnt) *180/CV_PI);
    ...
    // SORT(detections)
    qsort(detections, detections_count, sizeof(rbox_prob), detectionsR_comparator);
...
#endif // OPENCV
char* concat(const char *s1, const char *s2)  //字符串连接函数
{
                   ...
                   float hier_thresh, int dont_show, int ext_output, int save_labels)
{
    ...
    char *name_list = option_find_str(options, "names", "data/names.list");
    int names_size = 0;
    char **names = get_labels_custom(name_list, &names_size); //get_labels(name_list);  获取数据集的名称

    image **alphabet = load_alphabet();  //  // 加载data/labels/文件夹中所有的字符标签图片，目的？
    network net = parse_network_cfg_custom(cfgfile, 1); // set batch=1
    if(weightfile){
        load_weights(&net, weightfile);
    }
    ...
    while(1){
        ...
        int nboxes = 0;
        // dets 包含所有预测的结果，其中低于阈值prob为0
        detectionR *dets = get_network_rboxes(&net, im.w, im.h, thresh, hier_thresh, 0, 1, &nboxes, letterbox);
        if (nms) do_rnms_sort(dets, nboxes, l.classes, nms);
        draw_detectionsR_v3(im, dets, nboxes, thresh, names, alphabet, l.classes, ext_output);
    ...
    while(1){
        ...
        char pathRes[100];
        int i = 0;
        pch = strstr(input, "data");
        for (i = 0; i < strlen(input) - strlen(pch); i++)
        {
            pathRes[i] = input[i];
        }
        pathRes[i] = '\0';
        list *plist = get_paths(input);   //解析出所有图片名
        int N = plist->size;
        char predName[50];
        char **paths = (char **)list_to_array(plist);
        for (int i = 0; i < N; i++)
        {
            char * res = concat(pathRes, paths[i]);
            printf("%s \n", res);

            char* pos1 = strrchr(paths[i], '/');
            char* pos2 = strchr(paths[i], '.');
            char imgName[20] = { "" };
            strncpy(imgName, paths[i] + strlen(paths[i]) - strlen(pos1) + 1, strlen(pos1) - strlen(pos2) - 1);


           image im = load_image(res,0,0,net.c);
            int letterbox = 0;
            image sized = resize_image(im, net.w, net.h);
            layer l = net.layers[net.n-1];
            ...
            time= what_time_is_it_now();
            network_predict(net, X);
            printf("%s: Predicted in %f seconds.\n", res, (what_time_is_it_now()-time));
            int nboxes = 0;
           //  dets 包含所有预测的结果，其中低于阈值prob为0
            detectionR *dets = get_network_rboxes(&net, im.w, im.h, thresh, hier_thresh, 0, 1, &nboxes, letterbox);
            if (nms) do_rnms_sort(dets, nboxes, l.classes, nms);
            draw_detectionsR_v3(im, dets, nboxes, thresh, names, alphabet, l.classes, ext_output);
            sprintf(predName, "Predictions/pred_%s", imgName);  //sprinf_s  错误
            save_image(im, predName);
...
void run_detector(int argc, char **argv)
{
    ...
    int num_of_clusters = find_int_arg(argc, argv, "-num_of_clusters", 6);    //rot 6个anchor
