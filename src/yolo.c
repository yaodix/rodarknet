...
void train_yolo(char *cfgfile, char *weightfile)
{
    ...
    srand(time(0));
    char *base = basecfg(cfgfile);    //cfgfile 就是上面所说的cfg指向的字符串，返回cfg文件的名称，不包括 .cfg
    printf("%s\n", base);
    float avg_loss = -1;
    network net = parse_network_cfg(cfgfile);   //读取网络配置，解析了cfg文件
    if(weightfile){
        load_weights(&net, weightfile);
    }
    printf("Learning Rate: %g, Momentum: %g, Decay: %g\n", net.learning_rate, net.momentum, net.decay);
    //imgs是一次加载到内存中的图像数量，如果占用内存太大的话可以
    ...
    int side = l.side;          //side 就是论文中的划分网格的S
    int classes = l.classes;
    float jitter = l.jitter;    // 非均衡数据集处理：利用抖动(jittering)生成额外数据
    ...
    args.h = net.h;
    args.paths = paths;
    args.n = imgs;              //ly__n 就是一次加载到内存中的图片数量
    args.m = plist->size;   //ly__m是待训练的图片的总数量
